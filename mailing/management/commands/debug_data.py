    ...
    def handle(self, *args, **options):
        ...
        if clients:
            for client in clients:
                self.stdout.write(
                    f"ID: {client.id}, Email: {client.email}, Владелец: {client.owner.username}"
                )
        else:
        ...
        if messages:
            for message in messages:
                self.stdout.write(
                    f"ID: {message.id}, Тема: {message.subject}, Владелец: {message.owner.username}"
                )
        else:
        ...
        if mailings:
            for mailing in mailings:
                self.stdout.write(
                    f"ID: {mailing.id}, Тема: {mailing.message.subject}, Клиентов: {mailing.clients.count()}, Владелец: {mailing.owner.username}"
                )
        else:
