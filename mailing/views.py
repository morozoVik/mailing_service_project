...
def index(request):
    ...
    if request.user.is_authenticated:
        ...
        active_mailings = Mailing.objects.filter(
            owner=request.user, status=Mailing.STATUS_STARTED
        ).count()
        ...
        created_mailings = Mailing.objects.filter(
            owner=request.user, status=Mailing.STATUS_CREATED
        ).count()
        completed_mailings = Mailing.objects.filter(
            owner=request.user, status=Mailing.STATUS_COMPLETED
        ).count()

        # Последние рассылки
        recent_mailings = Mailing.objects.filter(owner=request.user).order_by(
            "-created_at"
        )[:5]
...
def message_delete(request, pk):
    ...
    if request.method == "POST":
        ...
        return redirect("mailing:message_list")
    return render(
        request, "mailing/message_confirm_delete.html", {"message": message_obj}
    )
