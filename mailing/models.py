...
class Mailing(models.Model):
    ...
    def is_active(self):
        ...
        now = timezone.now()
        return (self.status == self.STATUS_STARTED and
    ...
    def get_successful_attempts_count(self):
        ...
        return self.mailingattempt_set.filter(status=MailingAttempt.STATUS_SUCCESS).count()

    def get_failed_attempts_count(self):
        """Возвращает количество неуспешных попыток"""
        return self.mailingattempt_set.filter(status=MailingAttempt.STATUS_FAILED).count()
