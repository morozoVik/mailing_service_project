...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Создаем клиентов
        clients_data = [
            {'email': 'client1@example.com', 'full_name': 'Иван Иванов', 'comment': 'Постоянный клиент'},
            {'email': 'client2@example.com', 'full_name': 'Петр Петров', 'comment': 'Новый клиент'},
            {'email': 'client3@example.com', 'full_name': 'Мария Сидорова', 'comment': 'VIP клиент'},
        ]
        ...
        if created:
            ...
            self.stdout.write(self.style.SUCCESS(f'Создана рассылка ID: {mailing.id} с {len(clients)} клиентами'))
        else:
